                ...
                if (string.IsNullOrWhiteSpace(Guid) && _azureSynapseSettings.UseRelaxed == "1")
                {
                    ...
                    if (await rdVc2.ReadAsync(cancellationToken))
                    {
                        if (rdVc2 != null)
                        {
                            ...
                            _logger.LogInformation($"RESPONSE DETAIL: {_azureSynapseSettings.RelaxedQuery}; rdVc: UserId={Convert.ToString(rdVc2.GetValue(0))}, msg={rdVc2.GetString(1)}; request.Id={request.Id}");
