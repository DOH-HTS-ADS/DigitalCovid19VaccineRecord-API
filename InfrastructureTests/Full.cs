    ...
    public class Full
    {
        ...
        public async Task GeneralTest(int numDoses, int orgNameSize, int lotNumberSize = 20)
        {
            ...
            try
            {
                ...
                pngQr = await (new QrApiService(new AppSettings { QrCodeApi = "https://testdvrqrcode.azurewebsites.net/api/QRCreate" })).GetQrCodeAsync(shcs[0]);
        ...
        public static bool VerifySignature2(byte[] signature, byte[] message, AsymmetricKeyParameter publicKey)
        {
            ...
            verifier.Init(false, publicKey);
            BigInteger r = new(1, signature, 0, signature.Length / 2);
            BigInteger s = new(1, signature, signature.Length / 2, signature.Length / 2);
        ...
        private readonly static Random random = new((int)(DateTime.Now.Ticks % 1000000000));
